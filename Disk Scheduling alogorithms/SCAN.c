...
void main()
{
	...
	if(density1<density2)
	{
		...
		{
			total+=head_position-R[i].sector;
			printf("\n\n\t\tHead moved from sector %d to sector %d",head_position,R[i].sector);
			head_position=R[i].sector;
		}
		total+=head_position;
		printf("\n\n\t\tHead moved from sector %d to sector 0",head_position);
		head_position=0;
		for(i=density1;i<n;i++)
		{
			total+=R[i].sector-head_position;
			printf("\n\n\t\tHead moved from sector %d to sector %d",head_position,R[i].sector);
			head_position=R[i].sector;
		}
	...
	else
	{
		...
		total+=200-head_position;
		printf("\n\n\t\tHead moved from sector %d to sector 200",head_position);
		head_position=200;
		for(i=(density1-1);i>=0;i--)
		{
			total+=head_position-R[i].sector;
